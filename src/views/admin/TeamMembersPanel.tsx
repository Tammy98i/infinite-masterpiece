...
export function TeamMembersPanel() {
  ...
  return <section className="grid gap-6" aria-labelledby="team-members-admin-title" dir="rtl">
    <header className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-xs tracking-widest text-[#C8A24C] mb-2" dir="ltr">TEAM MEMBERS</p><h2 id="team-members-admin-title" className="text-2xl font-light">גלקסיית הצוות</h2>
        <p className="text-sm text-white/55 mt-2">ניהול האנשים, הפרופילים ורמות ההשפעה בסקשן הוובינר.</p></div>
      {!editing && <button type="button" className="px-5 py-3 border border-[#C8A24C]/50 rounded-full text-[#F7E7B5] min-h-11" onClick={() => { setNotice(''); setEditing('new'); }}>הוספת איש צוות לגלקסיה</button>}
    </header>
    <p className="text-xs text-white/55 border border-[#C8A24C]/25 rounded-xl p-4 leading-relaxed">ציוני ההשפעה הראשוניים הם ערכי פתיחה לצורכי תצוגה, לא מדידה מאומתת. ניתן לערוך אותם כאן. גודל הכוכב מתעדכן לפי הציון; המייסד נשאר השמש הגדולה במרכז. הפרטים הועתקו מהצוות הקיים פעם אחת, ומנוהלים מעתה כאן באופן עצמאי.</p>
    {error && <p role="alert" className="text-[#F7E7B5]">{error}</p>}
    {notice && <p role="status" className="text-[#F7E7B5]">{notice}</p>}
    {loading ? <p role="status">טוענים את הצוות…</p> : editing ? <TeamMemberForm member={editing === 'new' ? null : editing} onCancel={() => setEditing(null)} onSaved={member => {
      ...
      <div><h3 className="text-lg">{member.name}</h3><p className="text-sm text-white/60 mt-1">{member.role}</p><p className="text-xs text-[#C8A24C] mt-2">{TEAM_LEVEL_LABELS[member.hierarchy_level]} · השפעה {member.impact_score} · מסלול {member.orbit} · סדר {member.display_order} · {member.active ? 'מוצג באתר' : 'מוסתר'}</p></div>
      <button type="button" aria-label={`עריכת ${member.name}`} className="px-5 py-3 rounded-full border border-[#C8A24C]/40 text-sm min-h-11" onClick={() => { setNotice(''); setEditing(member); }}>עריכת פרופיל</button>
    </article>)}</div>}
  </section>;
}
...
function TeamMemberForm({ member, onSaved, onCancel }: { member: TeamMember | null; onSaved: (member: TeamMember) => void; onCancel: () => void }) {
  ...
  return <form className="grid gap-5 max-w-3xl" data-testid="team-member-form" onSubmit={async event => {
    ...
    <fieldset disabled={pending} className="grid gap-5">
      ...
      <div className="flex items-center gap-6 min-h-60 p-5 border border-[#C8A24C]/20 rounded-2xl overflow-hidden" aria-label="תצוגה מקדימה של גודל הכוכב">
        <span data-testid="star-size-preview" className="shrink-0 rounded-full border border-[#C8A24C] bg-[#C8A24C]/10 shadow-[0_0_25px_#c8a24c25]" style={{ width: starDiameter(draft), height: starDiameter(draft) }} />
        <p className="text-xs text-[#F7E7B5]">Higher Impact<br />Larger Star<br /><span className="block mt-2">{draft.impact_score} / 100</span></p>
      </div>
