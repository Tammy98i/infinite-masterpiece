...
export function SearchFilters({ value, onChange, activeCount }: { value: SearchFilterState; onChange: (value: SearchFilterState) => void; activeCount: number }) {
  ...
  const control = 'min-h-11 min-w-0 w-full rounded-xl border border-white/10 bg-[#0a0a0a] px-2 sm:px-3 text-xs sm:text-sm text-white/70 outline-none focus:border-[#C8A24C] sm:w-auto';
  return <div className="mb-6 rounded-2xl border border-white/10 bg-white/[0.025] p-3">
    <div className="grid grid-cols-2 items-center gap-2 sm:flex sm:flex-wrap">
      <span className="col-span-2 me-1 inline-flex items-center gap-2 text-sm text-white/50 sm:col-span-1"><SlidersHorizontal size={16} className="text-[#C8A24C]" />סינון</span>
      ...
      <select value={value.access} onChange={event => field('access', event.target.value)} className={control} aria-label="סינון לפי גישה"><option value="all">כל אפשרויות הגישה</option><option value="open">פתוח</option><option value="preview">טעימה</option><option value="locked">דורש מסלול</option></select>
      ...
      {activeCount > 0 ? <button type="button" onClick={() => onChange(EMPTY_FILTERS)} className="col-span-2 inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl px-3 text-sm text-[#C8A24C] hover:bg-[#C8A24C]/10 sm:col-span-1" aria-label="ניקוי כל המסננים"><X size={15} />ניקוי ({activeCount})</button> : null}
    </div>
  </div>;
}
