...
export function SiteSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-[14vh]">
    <button type="button" onClick={onClose} className="absolute inset-0 bg-[#010308]/85 backdrop-blur-md" aria-label="סגירת חיפוש" />
    <section role="dialog" aria-modal="true" aria-labelledby="site-search-title" className="relative w-full max-w-xl overflow-hidden rounded-3xl border border-[#C8A24C]/30 bg-[#06080d] shadow-2xl">
      ...
      <div className="flex items-center gap-3 border-b border-white/10 px-5">
        <Search size={19} className="text-[#C8A24C]" />
      ...
      <div className="max-h-[55vh] overflow-y-auto p-3">
        {results.length ? results.map(item => <Link key={item.to} to={item.to} onClick={onClose} className="group flex min-h-16 items-center gap-4 rounded-2xl px-4 py-3 text-right hover:bg-[#C8A24C]/10">
          <span className="min-w-0 flex-1"><strong className="block font-normal text-white">{item.title}</strong><span className="mt-1 block text-sm text-white/45">{item.description}</span></span><ArrowLeft size={17} className="text-white/25 transition-transform group-hover:-translate-x-1 group-hover:text-[#C8A24C]" />
