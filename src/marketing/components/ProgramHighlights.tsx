...
export function ProgramHighlights() { return <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{HIGHLIGHTS.map(item => <article key={item.days} className="glass-card p-6 text-right"><item.icon className="mb-5 h-6 w-6 text-[#C8A24C]" strokeWidth={1.4} /><p className="mb-2 text-[11px] tracking-[.15em] text-[#C8A24C]">{item.days}</p><h3 className="mb-2 text-lg text-white">{item.title}</h3><p className="text-sm font-light leading-relaxed text-white/50">{item.text}</p></article>)}</div>; }
