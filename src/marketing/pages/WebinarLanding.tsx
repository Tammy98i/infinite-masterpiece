...
import { WebinarExperience } from '../components/webinar-deck/WebinarExperience';
...
import '../components/webinar-deck/WebinarExperience.css';
...
export function WebinarLanding() {
  ...
  const [fitSeen, setFitSeen] = useState(false);
  ...
  const onSlideChange = useCallback((id: string) => {
    if (id !== 'webinar-fit' || fitSeen) return;
    setFitSeen(true);
  ...
  return (
    <WebinarExperience
      date={config.date}
      time={config.time}
      meta={`${config.location} · ${config.durationMinutes} דקות · ${WEBINAR_AUDIENCE_LABEL}`}
      ...
      slides={[
            ...
            <div className="webinar-editorial-grid">
              <section id="hosts" className="webinar-neural-slide">
                <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 text-center">
                  ...
                  <ul className="webinar-neural-feats webinar-deck-feats">
                    {eveningFeats.map((label) => (
                      <li key={label}>
                        <NeuralChevron />
                        <span>{label}</span>
                      </li>
                  ...
                  <ol className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    {WEBINAR_TASK_STEPS.map((item, stepIndex) => (
                      <li key={item.title} className="glass-card p-5">
                        <p className="text-[11px] text-[#b79043] mb-3">{String(stepIndex + 1).padStart(2, '0')}</p>
        ...
        },
        {
          ...
          children: (
            <div className="webinar-editorial-grid">
              <section id="webinar-fit" className="webinar-neural-slide">
        ...
        {
          id: 'webinar-register',
          title: eventEnded ? 'המחזור הבא' : 'הרשמה',
          aliases: ['webinar-register-bottom'],
          children: (
            <div className="webinar-editorial-grid">
