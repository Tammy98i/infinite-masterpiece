...
import { userFromToken } from './authService.js';
...
async function fetchSupabaseUser(accessToken: string): Promise<SupabaseUserPayload> {
  ...
  if (!res.ok) {
    throw Object.assign(new Error('טוקן Supabase לא תקין'), { status: 401 });
  }
...
function upsertLocalUserFromSupabase(input: {
  ...
  if (!email.includes('@')) {
    throw Object.assign(new Error('חסר אימייל בחשבון Supabase'), { status: 400 });
  }
...
export async function syncSupabaseSession(accessToken: string, fullNameHint = '') {
  if (!supabaseConfigured()) {
    throw Object.assign(new Error('Supabase לא מוגדר בשרת'), { status: 503 });
  }
