...
- `SiteBackdropLayout` in `Layout.tsx` wraps both public and library routes in `App.tsx` with one persistent static `SkyBackdrop` (`/webinar-background.jpg`) and its readability overlays. Do not add per-view backdrops.
