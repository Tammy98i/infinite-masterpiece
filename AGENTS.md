...
- `SiteBackdropLayout` still provides the persistent static `SkyBackdrop` (user-provided blue Milky Way starfield hosted on `media.base44.com`, `ce298813f_image.png`) behind every route, covered by one semi-transparent dark layer with a subtle backdrop blur. The three planet clips are scoped to the homepage hero and unmount on navigation.
