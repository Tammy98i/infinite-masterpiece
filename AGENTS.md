...
- `SiteBackdropLayout` still provides the persistent static `SkyBackdrop` (user-provided Milky Way starfield hosted on `media.base44.com`, `8726a79f8_image.png`) behind every route, softened by a light black blur overlay. The three planet clips are scoped to the homepage hero and unmount on navigation.
